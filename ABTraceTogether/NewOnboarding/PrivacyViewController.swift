...
class PrivacyViewController: BaseViewController {
    ...
    override func viewDidAppear(_ animated: Bool) {
        ...
        if privacyConcented == true {
            iAgreeSwitch.isOn = true
            iAgreeSwitchChanged(self)
        } else {
            iAgreeSwitch.isOn = false
            iAgreeSwitchChanged(self)
        }
    }
}
...
extension PrivacyViewController {
    ...
    @objc
    func privacyPolicyOnlineButtonAction(_ sender: UIButton) {
        showSafaiView(with: privacyLink)
    }

    @objc
    func faqButtonAction(_ sender: UIButton) {
        showSafaiView(with: privacyFaqLink)
    }

    @objc
    func iAgreeSwitchChanged(_ sender: Any) {
        privacyConcented = iAgreeSwitch.isOn
        bottomButton.isEnabled = iAgreeSwitch.isOn
...
extension PrivacyViewController {
    ...
    private func generateContent(in parentView: UIStackView) {
        ...
        detailMessageLabel1.setAttributedEmailLink(
            ...
            linkurl: UserDefaults.standard.string(forKey: privacyUrlKey) ?? "https://privacyUrl",
            middleText: "\n\n" + NSLocalizedString(
            ...
            secondLinkurl: UserDefaults.standard.string(forKey: faqUrlKey) ?? "https://myorg/help",
            secondMiddleText: "\n\n" + NSLocalizedString(
            ...
            firstEmailLink: UserDefaults.standard.string(forKey: helpEmailKey) ?? "help@myorg",
            endText: ".\n\n" +
                NSLocalizedString(
            ...
            secondEmailLink: UserDefaults.standard.string(forKey: helpEmailKey) ?? "help@myorg"
        )
        ...
        iAgreeLabel.leadingAnchor.constraint(equalTo: parentView.leadingAnchor, constant: 20).isActive = true
        iAgreeLabel.trailingAnchor.constraint(equalTo: parentView.trailingAnchor, constant: -35).isActive = true
        ...
        newView.addSubview(iAgreeSwitch)

        iAgreeSwitch.translatesAutoresizingMaskIntoConstraints = false
        iAgreeSwitch.trailingAnchor.constraint(equalTo: parentView.trailingAnchor, constant: -20).isActive = true
        iAgreeSwitch.centerYAnchor.constraint(equalTo: iAgreeLabel.centerYAnchor, constant: 0).isActive = true
        iAgreeSwitch.addTarget(self, action: #selector(iAgreeSwitchChanged), for: .valueChanged)
